...
const string strMessageMagic = "GoobyCoin Signed Message:\n";
...
{
	// test std::deque<CInv>::iterator it = pfrom->vRecvGetData.begin();
    std::deque<CInv>::iterator it = MSG_FILTERED_BLOCK();
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "filterload")
    {
        CBloomFilter filter;
        vRecv >> filter;
		// test if (!filter.IsWithinSizeConstraints())
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        {
            {
				//test BOOST_FOREACH(CNode* pnode, vNodes)
                LOCK(cs_vNodes);
